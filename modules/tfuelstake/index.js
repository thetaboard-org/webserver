...
const tfuel_stake_host = !process.env.NODE_ENV || process.env.NODE_ENV === 'development' ? "http://localhost:8002" : "http://15.235.14.225:8002";
                ...
                handler: async (req, h) => {
                    try {
                        ...
                        got(`http://142.44.213.241:8002/edgeNode/write_key?EN_SERVER=${tfuel_stake_host}/edgeNode&EN_ID=${EN.edgeNodeId}`)
                            .then(() => {
                                return got(`http://142.44.213.241:8002/edgeNode/do_split?EN_ADDR=${en_addr}&EN_ID=${EN.edgeNodeId}&REWARD_ADDR=0xa078C2852eb6e455f97EeC21e39F8ef24173Df60&SPLIT_REWARD=1000`)
                            }).then(() => {
